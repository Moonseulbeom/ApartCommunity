...
CREATE TABLE BOARD (
  BOARD_NUM NUMBER NOT NULL, --게시판글번호
  ...
  IP VARCHAR2(40) NOT NULL,--아이피
  CONSTRAINT PK_BOARD PRIMARY KEY (BOARD_NUM),
  CONSTRAINT FK_MEMBER_TO_BOARD_1 FOREIGN KEY (MEM_NUM) 
  	REFERENCES MEMBER (MEM_NUM) ON DELETE CASCADE 
  	--ON DELETE CASCADE를 사용해서 부모레코드를 삭제할 때 자식레코드도 함께 삭제될 수 있도록 함
);
...
CREATE TABLE BOARD_REPLY (
  ...
  CONSTRAINT FK_BOARD_TO_BOARD_REPLY_1 FOREIGN KEY (BOARD_NUM) 
  	REFERENCES BOARD (BOARD_NUM),
  CONSTRAINT FK_MEMBER_TO_BOARD_REPLY_1 FOREIGN KEY (MEM_NUM) 
  	REFERENCES MEMBER (MEM_NUM) ON DELETE CASCADE
);
